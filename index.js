...
function pickBestAudioFormat(info) {
  ...
  const preferredIds = ['251', '140', '250', '249'];

  for (const formatId of preferredIds) {
    const match = formats.find(format =>
      format &&
      String(format.format_id) === formatId &&
      format.url &&
      format.acodec &&
      format.acodec !== 'none'
    );
...
async function getDirectPlayableUrl(songUrl) {
  ...
  const selectedFormat = pickBestAudioFormat(info);
  ...
  console.log(
    `Selected format ${selectedFormat.format_id} (${selectedFormat.ext || 'unknown'}, ${selectedFormat.abr || selectedFormat.tbr || 'n/a'}k)`
  );
...
async function playSong(guild, song, attempt = 0, startSeconds = 0) {
  ...
  try {
    ...
    ffmpegArgs.push('-i', String(streamData.url).trim());
